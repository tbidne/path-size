...
display (MkDisplayConfig {color, format, reverseSort}) spd =
  ...
  where
    ...
    -- "Size  " == 6
    -- Sep x 3 == 3 x 3 == 9
    -- total constant space = 6 + 9 = 15
    --
    -- TODO: This calculation appears wrong for windows. It would be nice
    -- to know why.
    hyphens = T.replicate (15 + padDir + padFile + padPath) "-"
...
-- Hence we normalize it first to account for grapheme clusters.
--
...
-- byte boundaries. Hence a simpler solution (that ignores grapheme clusters)
-- could use os-string length but would need to guard the GHC and os-string
-- version.
